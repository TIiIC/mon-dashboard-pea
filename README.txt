...
2. **"Table Produit"** : Liste des produits actuellement détenus
   - Données calculées et agrégées

3. **"Stock Tickers"** : Référentiel des produits avec méthodes de scraping
   - Configuration pour récupération données live/historique

4. **"Data Live"** : Cours en temps réel via formule GOOGLEFINANCE()
   - Colonne B (index 1) : ID_perso (nom de la feuille historique associée)
   - Formules Google Finance pour cours actuels

5. **Feuilles individuelles par produit** : Historique des cours
...
```javascript
{
  ...
  "live": Array<LiveProduct>,              // Produits en portefeuille (Table Produit)
...
**LiveProduct** (feuille "Table Produit")
```javascript
// Structure variable selon colonnes de la feuille
// Probablement similaire à LiveData avec champs calculés additionnels
// À utiliser avec prudence - préférer reconstructLive() côté frontend
{
  liste_produits: String,            // Nom du produit
  ticker: String,                    // Code ticker
  dividende: Number,                 // Dividendes perçus (si applicable)
  // ... autres champs calculés par formules Google Sheets
}
```
...
⚠️ **IMPORTANT** : Le frontend ne fait **PAS confiance** à la feuille "Table Produit" (qui peut contenir des erreurs de formules ou être obsolète).
...
- **Dividendes** : Récupérés depuis `result.live` via `getProductDividend()`
...
**Fonction `getProductDividend(item, resultLive)`**
```javascript
// Même logique de fallback pour récupérer dividendes
...
### Flux de données
✅ Source de vérité : Google Sheets (5 feuilles + feuilles produits)  
✅ API : doGet() lecture, doPost() écriture (3 opérations)  
✅ Frontend recalcule tout via reconstructLive() (pas de confiance en formules GS)  
...
**Problème** : La feuille "Table Produit" peut contenir des erreurs de formules
**Solution** : **TOUJOURS** utiliser `reconstructLive()` - ne jamais faire confiance à `result.live` directement
...
### 8. Calculs de performance incorrects
...
2. **Identifier les sources de vérité**
   ...
   - ⚠️ NE PAS utiliser "Table Produit" directement
